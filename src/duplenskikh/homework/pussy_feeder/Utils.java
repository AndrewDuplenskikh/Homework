package duplenskikh.homework.pussy_feeder;

public class Utils {
    public static final String ENTRY_MESSAGE = "Приветствую Вас в игре 'Накорми киску'!";
    public static final String COMMANDS_MESSAGE = "Выберите действие: " +
            "\n 1 - Накормить одного кота" +
            "\n 2 - Накормить всех котов" +
            "\n 3 - Накормить четных котов" +
            "\n 4 - Накормить нечетных котов" +
            "\n 5 - Накормить всех котов полностью" +
            "\n 666 - Выйти";
    public static final String QUESTION_FOOD_MESSAGE = "Сколько еды положить? (Или отнять, указав отрицательное значение)";
    public static final String QUESTION_CAT_MESSAGE = "Выберите кота (введите номер миски от 1 до 7)";
    public static final String ERROR_MESSAGE = "Такой команды не существует";
    public static final String EXIT_MESSAGE = "До скорой встречи!";
}
